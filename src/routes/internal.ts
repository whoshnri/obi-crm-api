  ...
  .post("/jobs/run", async (c) => {
    return handleRoute(c, async () => {
      ...
      if (payload.job_type === "opportunity_event") {
        ...
        if (!event) {
          throw new HttpError(`Opportunity event with cronJobId ${payload.job_id} not found`, 404);
        }
      ...
      } else if (payload.job_type === "participant_event") {
        const event = await prisma.event.findUnique({
          where: { id: payload.job_id },
        });
        if (!event) {
          throw new HttpError(`Participant event with id ${payload.job_id} not found`, 404);
        }
